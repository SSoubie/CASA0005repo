...
* [Chapter 13 "Spatial Autocorrelation"](https://mgimond.github.io/Spatial/chp13_0.html) from Intro to GIS and Spatial Analysis by Gimond (2019).
...
LondonWardsMerged <- LondonWardsMerged %>% 
  ...
  distinct(GSS_CODE, .keep_all = T) %>%
  select(GSS_CODE, ward_name, average_gcse_capped_point_scores_2014)
...
points_sf_joined <- LondonWardsMerged%>%
  ...
  select(density, ward_name, gss_code, n, average_gcse_capped_point_scores_2014)
...
* we are looking at the product of deviaitions from the global mean (subtracting our central value $(z_i - \bar{z})$ and neighbour $(z_j - \bar{z})$ from the global mean individually, then multiplying them).
...
It returns several columns, of most interest is the Z score. A Z-score is **how many standard deviations a value is away (above or below) from the mean**. This allows us to state if our value is significantly different than expected value at this location considering the neighbours. 
